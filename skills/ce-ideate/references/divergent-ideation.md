...
**When two variants are active at once, the surface and the budget are decided separately.** Whichever variant decides the *surface* picks the frames and the agent count; tactical contributes only **its dials** (`references/scope-gates.md` Phase 0.5) — it never repacks another variant's fleet.

| Both active | Frames | Agents | Volume / reads |
|---|---|---|---|
| issue-tracker + tactical | theme frames (issue-tracker decides the surface) | 4 (issue-tracker keeps its fleet) | tactical's dials |
